...
def main():
    ...
    sample_items = [
        ...
        {
            'description': 'Providing and fixing M.S. reinforcement',
            'unit': 'Kg',
            'quantity': '5,420.00',
            'rate': '52.00',
            'amount': '2,81,840.00'
    ...
    st.info("Using sample work items for demonstration. Integrate with your Excel data processing.")
    ...
    with col1:
        subtotal = st.text_input("Subtotal (₹)", value="41,26,215.00")
    
    with col2:
        premium = st.text_input("Tender Premium (₹)", value="2,06,310.75")
    
    with col3:
        gst = st.text_input("GST Amount (₹)", value="7,81,854.64")
    
    grand_total = st.text_input("Grand Total (₹)", value="51,14,380.39")
    
    # Footer
    footer_text = st.text_input(
        "Footer Text",
        value="This is a computer-generated document | Page 1 of 1"
    )
    ...
    if st.button("🎯 Generate PDF", type="primary", use_container_width=True):
        ...
        if pdf_path:
            ...
            st.info(f"**File Size:** {file_size:.2f} KB | **Format:** A4 {config['orientation'].title()}")
